...
module AESL_axi_master_gmem (
    ...
    TRAN_gmem_BID,
    TRAN_gmem_BUSER,
    TRAN_gmem_feature_in,
    TRAN_gmem_W,
    TRAN_gmem_bias,
    TRAN_gmem_feature_out,
    ready,
    done
    );

//------------------------Parameter----------------------
`define TV_IN_gmem "./c.Conv.autotvin_gmem.dat"
`define TV_OUT_gmem "./rtl.Conv.autotvout_gmem.dat"
...
parameter gmem_mem_depth = 32'd 4;
...
output [32 - 1 : 0] TRAN_gmem_feature_in;
output [32 - 1 : 0] TRAN_gmem_W;
output [32 - 1 : 0] TRAN_gmem_bias;
output [32 - 1 : 0] TRAN_gmem_feature_out;
input ready;
input done;
...
reg RLAST_tmp = 0;
reg RVALID_tmp = 0;
reg [32 - 1 : 0] feature_in = 0;
reg [32 - 1 : 0] W = 0;
reg [32 - 1 : 0] bias = 0;
reg [32 - 1 : 0] feature_out = 0;
reg [gmem_DATA_BITWIDTH - 1 : 0] gmem_mem [0 : gmem_mem_depth * mem_page_num  - 1]; 
...
assign    TRAN_gmem_feature_in = feature_in;
assign    TRAN_gmem_W = W;
assign    TRAN_gmem_bias = bias;
assign    TRAN_gmem_feature_out = feature_out;
...
initial begin : initialize_offset
  ...
  feature_in <= 0 * DATA_byte_num;
  W <= 1 * DATA_byte_num;
  bias <= 2 * DATA_byte_num;
  feature_out <= 3 * DATA_byte_num;
end

initial begin : initialize_gmem_mem
  integer i;
  for (i = 0; i < gmem_mem_depth * mem_page_num; i = i + 1) begin
      gmem_mem[i] = 0;
  end
end
...
always @(posedge clk or reset) begin 
    if(reset === 0) begin 
      ...
      if(TRAN_gmem_AWVALID && TRAN_gmem_AWREADY && (FIFO_AW_req_ptr_w == FIFO_DEPTH - 1)) 
          FIFO_AW_req_flag <= 1; 
      # 0.4;
...
always @(posedge clk or reset) begin 
    if(reset === 0) begin 
      ...
      if(TRAN_gmem_ARVALID && TRAN_gmem_ARREADY && (FIFO_AR_req_ptr_w == FIFO_DEPTH - 1)) 
          FIFO_AR_req_flag <= 1; 
      # 0.4;
...
always @(posedge clk or reset) begin 
    if(reset === 0) begin 
      ...
      if(TRAN_gmem_WVALID && TRAN_gmem_WREADY && (FIFO_WDATA_req_ptr_w == FIFO_DEPTH - 1)) 
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > gmem_mem_depth) begin
                $display ("C:/Users/Caiyujie/Desktop/gongkaike/hls/conv_core/solution1/sim/verilog/AESL_axi_master_gmem.v: Write request address %d exceed AXI master gmem array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, gmem_mem_depth); 
                $finish;
            end
                    ...
                    if (FIFO_WDATA_req_empty != 1) begin 
                        ...
                        for (i = 0 ; i < data_byte_size; i = i + 1) begin
                            if (FIFO_WSTRB_tmp[i] == 1) begin 
                                ...
                                for (j=(i*8); j< (i+1)*8 ; j= j+1) begin
                                    WDATA_tmp[j] = gmem_mem [(FIFO_AW_req_transaction_tmp % mem_page_num) * gmem_mem_depth + FIFO_AW_req_ADDR_tmp/data_byte_size + counter][j];
                                end
                            end
                        end
                        gmem_mem [(FIFO_AW_req_transaction_tmp % mem_page_num) * gmem_mem_depth + FIFO_AW_req_ADDR_tmp/data_byte_size + counter] <= WDATA_tmp;
                    ...
                    if (FIFO_WDATA_req_empty != 1) begin 
                        ...
                        for (i = 0 ; i < data_byte_size; i = i + 1) begin
                            if (FIFO_WSTRB_tmp[i] == 1) begin 
                                ...
                                for (j=(i*8); j< (i+1)*8 ; j= j+1) begin
                                    WDATA_tmp[j] = gmem_mem [(FIFO_AW_req_transaction_tmp % mem_page_num) * gmem_mem_depth + FIFO_AW_req_ADDR_tmp/data_byte_size][j];
                                end
                            end    
                        end
                        gmem_mem [(FIFO_AW_req_transaction_tmp % mem_page_num) * gmem_mem_depth + FIFO_AW_req_ADDR_tmp/data_byte_size] <= WDATA_tmp;
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > gmem_mem_depth) begin
                $display ("C:/Users/Caiyujie/Desktop/gongkaike/hls/conv_core/solution1/sim/verilog/AESL_axi_master_gmem.v: Read request address %d exceed AXI master gmem array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, gmem_mem_depth); 
                $finish;
            end
            ...
            if (FIFO_AR_req_BURST_tmp === 1) begin
                ...
                while (counter != input_length + 1) begin
                    RDATA_tmp <= gmem_mem [(FIFO_AR_req_transaction_tmp % mem_page_num) * gmem_mem_depth + FIFO_AR_req_ADDR_tmp/data_byte_size +   counter] ;
                    RVALID_tmp <= 1;
                    RRESP_tmp <= 0;
                ...
                input_length = FIFO_AR_req_LEN_tmp;
                while (counter != input_length + 1) begin
                    RDATA_tmp <= gmem_mem [(FIFO_AR_req_transaction_tmp % mem_page_num) * gmem_mem_depth + FIFO_AR_req_ADDR_tmp/data_byte_size ] ;
                    RVALID_tmp <= 1;
                    RRESP_tmp <= 0;
                    RLAST_tmp <= 1;
                    @(posedge clk);
...
initial begin : write_file_proc 
  ...
  while(1) begin 
      ...
      for(i = 0; i < (gmem_mem_depth - gmem_mem_depth % factor); i = i + 1) begin 
          if (factor == 4) begin
              if (i%factor == 0) begin
                  mem_tmp = gmem_mem[base_page + i/factor][7:0];
              end
              if (i%factor == 1) begin
                  mem_tmp = gmem_mem[base_page + i/factor][15:8];
              end
              if (i%factor == 2) begin
                  mem_tmp = gmem_mem[base_page + i/factor][23:16];
              end
              if (i%factor == 3) begin
                  mem_tmp = gmem_mem[base_page + i/factor][31:24];
              end
              $fdisplay(fp,"0x%x",mem_tmp);
          end
          if (factor == 2) begin
              if (i%factor == 0) begin
                  mem_tmp = gmem_mem[base_page + i/factor][15:0];
              end
              if (i%factor == 1) begin
                  mem_tmp = gmem_mem[base_page + i/factor][31:16];
              end
              $fdisplay(fp,"0x%x",mem_tmp);
          end
          if (factor == 1) begin
              $fdisplay(fp,"0x%x",gmem_mem[base_page + i]);
          end
      end 
      if (factor == 4) begin
          if ((gmem_mem_depth - 1) % factor == 2) begin
              $fdisplay(fp,"0x%x",gmem_mem[base_page + gmem_mem_depth/factor][7:0]);
              $fdisplay(fp,"0x%x",gmem_mem[base_page + gmem_mem_depth/factor][15:8]);
              $fdisplay(fp,"0x%x",gmem_mem[base_page + gmem_mem_depth/factor][23:16]);
          end
      ...
      if (factor == 2) begin
          if ((gmem_mem_depth - 1) % factor == 0) begin
              $fdisplay(fp,"0x%x",gmem_mem[base_page + gmem_mem_depth/factor][15:0]);
          end
      end
