...
def readbinfile(filename,size): #read bin
    f = open(filename, "rb")
    z=[]
    for j in range(size):
        data = f.read(4)
        data_float = struct.unpack("f", data)[0] #float 32bit
        z.append(data_float)
    f.close()
    z = np.array(z)
    return z

def RunConv(conv,Kx,Ky,Sx,Sy,mode,relu_en,feature_in,W,bias,feature_out): #卷积驱动
...
def RunPool(pool,Kx,Ky,mode,feature_in,feature_out): #池化驱动
...
# 持续读取摄像头数据
    ...
    #   cv2.imshow("screen_title", frame)
        ...
        cv2.imwrite("/home/xilinx/overlay/mnist/data/1.jpg", frame)
        break
    # 释放资源      
    capture.release()
    ...
    for i in range(IN_HEIGHT1):
        for j in range(IN_WIDTH1):
            for k in range(IN_CH1):
                image[i][j][k]=(255-image1[i][j])/255 #归一化
        ...
        MAX = h_fc2[0][0][0] #结果概率遍历，获得最大概率的结果
        result=0
